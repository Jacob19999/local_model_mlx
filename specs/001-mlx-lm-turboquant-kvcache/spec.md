...
- Q: What front-end experience is required? → A: The modern MLX UI is the required local user interface shell.
...
### User Story 2 - Enable TurboQuant runs (Priority: P2)
...
### User Story 3 - Preserve safe fallback behavior (Priority: P3)
...
### User Story 4 - Access the runtime from UI and third-party clients (Priority: P4)

As a user or tool integrator, I can access the same local model service from the modern MLX UI and from third-party clients through a standard API so there is one consistent local runtime surface.
...
**Independent Test**: Can be fully tested by launching the local UI and by sending one request from a third-party client to the local API, then confirming both use the same available models and generation behavior.

**Acceptance Scenarios**:

1. **Given** the local runtime is available, **When** the user opens the modern MLX UI, **Then** they can choose a supported model and start a generation session through the same underlying local service.
...
- **FR-009**: The system MUST provide the modern MLX UI as a local interface over the same runtime workflows exposed through the CLI.
...
- The local UI is a modern MLX-style shell over the same runtime workflows rather than a separate inference implementation.
